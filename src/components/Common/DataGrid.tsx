...
export const CustomNoOnAssetOverlay = () => {
  return (
    <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '20px', zIndex: '999', position: 'relative' }}>
      <Typography variant='p_lg'>No nft to display. </Typography>
    </Box>
  )
}
...
export const CellTicker: React.FC<TickerType> = ({ address, name }) => (
  <Box display="flex" justifyContent="flex-start">
    ...
    <Box display='flex' ml='10px' sx={{ flexDirection: { xs: 'column', md: 'row' }, alignItems: { xs: 'flex-start', md: 'center' } }}>
      {address && <Box sx={{ maxWidth: '200px', textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden' }}>
        <Typography variant='p_lg'>{address}</Typography>
      </Box>}
      <Box sx={{ color: '#8988a3', marginLeft: { xs: '0px', md: '10px' } }}>
        <Typography variant='p_lg'>{name}</Typography>
      </Box>
    </Box>
  </Box>
)
